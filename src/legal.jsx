...
function LegalFooter() {
  return <footer className="legal-footer"><nav aria-label="Legal"><a href="/">Home</a><a href="/privacy">Privacy Policy</a><a href="/terms">Terms</a></nav><p>© 2026 Roast My Startup.</p></footer>;
}
...
function PrivacyPolicy() {
  return <>
    ...
    <section><h2>1. Who runs this thing?</h2><p>Roast My Startup is an independent project. It is not presented as a limited liability company.</p></section>
    ...
    <section><h2>5. Analytics, monitoring, and abuse protection</h2><h3>PostHog</h3><p>PostHog records page views, page exits, and product events such as starting, submitting, completing, failing, or sharing a roast. Event properties are limited to coarse idea and background length buckets, model name, and sharing channel. Autocapture and session recording are disabled. We do not intentionally send startup text, founder names, locations, or other form contents to PostHog. PostHog may use browser storage and receive ordinary technical information required to process analytics. See <a href="https://posthog.com/privacy" target="_blank" rel="noreferrer">PostHog's privacy policy</a>.</p><h3>Vercel Web Analytics</h3><p>Vercel Web Analytics records aggregated page information including timestamp, page URL, referrer, coarse geolocation, browser, operating system, and device type. Vercel says these analytics do not use third party cookies or associate page views with an identifiable person or IP address. See <a href="https://vercel.com/docs/analytics/privacy-policy" target="_blank" rel="noreferrer">Vercel Analytics privacy information</a>.</p><h3>Sentry</h3><p>Sentry receives error reports from the website and roast function, including technical stack information, page route, runtime environment, and limited browser or device context. Startup submissions and form contents are deliberately removed from error events. Sentry is used to diagnose failures and send operational alerts. See <a href="https://sentry.io/privacy/" target="_blank" rel="noreferrer">Sentry's privacy policy</a>.</p><h3>Vercel BotID and firewall</h3><p>Vercel BotID runs an invisible browser challenge when you submit a roast. Vercel processes browser signals to distinguish people from automated traffic. The Vercel firewall also uses the requesting IP address to limit repeated calls to the roast endpoint. These controls are used to prevent abuse and unexpected generation costs. See <a href="https://vercel.com/docs/botid" target="_blank" rel="noreferrer">Vercel BotID information</a> and <a href="https://vercel.com/docs/vercel-firewall/vercel-waf/rate-limiting" target="_blank" rel="noreferrer">Vercel rate limiting information</a>.</p></section>
    ...
    <section><h2>8. Deletion and your choices</h2><p>You can delete the browser copy of a roast by starting another roast, closing the session, or clearing this site's browser data. You can block analytics using browser privacy controls or content blockers, although totals may become less accurate.</p><p>The site has no accounts or application database containing submissions, so there is no server side submission record for the site to delete. Analytics are anonymous or pseudonymous and may not be linkable to an individual visitor. Provider security records may be retained where required for safety, fraud prevention, or law.</p></section>
...
function Terms() {
  return <>
    ...
    <section><h2>10. Updates</h2><p>We may update these Terms as the service changes. Continued use after an update means the revised Terms apply from their effective date.</p></section>
  </>;
}
